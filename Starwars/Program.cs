...
namespace Starwars
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //string[] wantedPlanets = { "Mon Cala", "Malastare", "Mirial", "Muunilinst", "Mustafar", "Mygeeto" };
            ...
            // Opgave 3
            List<Planet> allPlanetsHavingNameLengthBetween6And15 = planets.Where(x => x.Name.Length > 6 && x.Name.Length < 15).ToList();

            foreach(Planet p in allPlanetsHavingNameLengthBetween6And15)
            {
                Console.WriteLine("Name: " + p.Name);
            }
            ...
            List<Planet> allPlanetsHavingAAsSecondCharAndEndingWithE = planets.Where(x => x.Name[1] == 'a' && x.Name[x.Name.Length - 1] == 'e').ToList();

            foreach(Planet p in allPlanetsHavingAAsSecondCharAndEndingWithE)
            {
                Console.WriteLine("Name: " + p.Name);
            }
            ...
            List<Planet> allPlanetsWithARotationPeriodGreaterThan40 = planets.Where(x => x.RotationPeriod > 40).OrderBy(x => x.RotationPeriod).ToList();

            foreach(Planet p in allPlanetsWithARotationPeriodGreaterThan40)
            {
                Console.WriteLine("Name: " + p.Name + ", RotationPeriod: " + p.RotationPeriod);
            }
            ...
            List<Planet> allPlanetsWithARotationPeriodBetween10And20 = planets.Where(x => x.RotationPeriod > 10 && x.RotationPeriod < 20).OrderBy(x => x.Name).ToList();

            foreach(Planet p in allPlanetsWithARotationPeriodBetween10And20)
            {
                Console.WriteLine("Name: " + p.Name + ", RotationPeriod: " + p.RotationPeriod);
            }
            ...
            List<Planet> allPlanetsWithARotationPeriodGreaterThan30 = planets.Where(x => x.RotationPeriod > 30).OrderBy(x => x.Name).ThenBy(x => x.RotationPeriod).ToList();

            foreach(Planet p in allPlanetsWithARotationPeriodGreaterThan30)
            {
                Console.WriteLine("Name: " + p.Name + ", RotationPeriod: " + p.RotationPeriod);
            }
            ...
            List<Planet> allPlanetsWithSurfaceWaterGreaterThan0 = planets.Where(x => x.SurfaceWater > 0).OrderByDescending(x => x.SurfaceWater).ToList();

            foreach(Planet p in allPlanetsWithSurfaceWaterGreaterThan0)
            {
                Console.WriteLine("Name: " + p.Name + ", RotationPeriod: " + p.RotationPeriod);
            }
            ...
            List<Planet> allPlanetsAndTheirSquareKilometerPerResident = planets.Where(x => x.Diameter > 0 && x.Population > 0).OrderBy(x => ((Math.PI  * Math.Pow(x.Diameter, 2)/x.Population))).ToList();

            foreach(Planet p in allPlanetsAndTheirSquareKilometerPerResident)
            {
                var km2PerPerson = ((Math.PI * Math.Pow(p.Diameter, 2)) / p.Population);
                Console.WriteLine("Name: " + p.Name + ", Diameter: " + p.Diameter + ", km^2 per person: " + km2PerPerson);
            }
            ...
            List<Planet> allPlanetsWhereRotationPeriodIsAbove0 = planets.Where(x => x.RotationPeriod > 0).ToList();

            List<Planet> allPlanetsExceptThoseWhereRotationPeriodIsAbove0 = planets.Except(allPlanetsWhereRotationPeriodIsAbove0).ToList();

            foreach(Planet p in allPlanetsExceptThoseWhereRotationPeriodIsAbove0)
            {
                Console.WriteLine("Name: " + p.Name);
            }
            ...
            List<Planet> allPlanetsWithRainForrestTerrain = planets.Where(x => x.Terrain != null && x.Terrain.Contains("rainforests")).ToList();

            List<Planet> unionedPlanets = allPlanetsBeginningWithAOrEndingOnS.Union(allPlanetsWithRainForrestTerrain).ToList();

            foreach(Planet p in unionedPlanets)
            {
                Console.WriteLine("Name: " + p.Name);
            }
            ...
            List<Planet> allPlanetsWithDesertTerrain = planets.Where(x => x.Terrain != null && x.Terrain.Contains("deserts")).ToList();

            foreach(Planet p in allPlanetsWithDesertTerrain)
            {
                Console.WriteLine("Name: " + p.Name);
            }
            ...
            List<Planet> allPlanetsWithSwampTerrain = planets.Where(x => x.Terrain != null && (x.Terrain.Contains("swamps") || x.Terrain.Contains("swamp"))).OrderBy(x => x.RotationPeriod).ThenBy(x => x.Name).ToList();

            Console.WriteLine("Opgave14");

            foreach(Planet p in allPlanetsWithSwampTerrain)
            {
                Console.WriteLine("Name: " + p.Name + ", RotationPeriod: " + p.RotationPeriod);
            }
            ...
            Regex reg = new Regex("([aeiuoyæøåe])\\1");
            List<Planet> allPlanetsWithDoubleVowelInTheName = planets.Where(x => reg.Matches(x.Name).Count > 0).ToList();


            foreach(Planet p in allPlanetsWithDoubleVowelInTheName)
            {
                Console.WriteLine("Name: " + p.Name);
            }
            ...
            Regex reg2 = new Regex("(kk|ll|rr|nn){1,}");

            List<Planet> allPlanetsContainingKKOrLLOrRROrNN = planets.Where(x => reg2.Matches(x.Name).Count > 0).OrderByDescending(x => x.Name).ToList();

            foreach(Planet p in allPlanetsContainingKKOrLLOrRROrNN)
            {
                Console.WriteLine("Name: " + p.Name);
            }
